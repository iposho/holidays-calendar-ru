...
- [📅 Экспорт в ICS](#-экспорт-в-формате-ics-icalendar)
...
# Экспорт календаря в формате ICS
curl "https://calendar.kuzyak.in/api/calendar/2023/ics"
```
...
- **Экспорт в ICS**: Календари в стандартном формате iCalendar для импорта в любые календарные приложения.
...
### 📅 Экспорт в формате ICS (iCalendar)
```http
GET /api/calendar/{year}/ics
```

Возвращает календарь праздничных дней в стандартном формате iCalendar (.ics), который можно импортировать в любые календарные приложения (Google Calendar, Outlook, Apple Calendar и др.).

**Особенности ICS файлов:**
- Содержат все праздничные и сокращенные дни года
- Совместимы со стандартом RFC 5545
- Включают метаданные о календаре (название, часовой пояс)
- Каждое событие содержит название праздника и категорию

**Пример использования:**
```bash
# Скачать календарь 2024 года
curl "https://calendar.kuzyak.in/api/calendar/2024/ics" -o calendar-2024.ics

# Импортировать в Google Calendar или другое приложение
```
...
### 📅 ICS (iCalendar) экспорт
```bash
# Скачать календарь праздников 2024 года
curl "https://calendar.kuzyak.in/api/calendar/2024/ics" -o holidays-2024.ics

# Импортировать в Google Calendar через веб-интерфейс
# Или использовать в календарных приложениях
```

### 🔗 Прямые ссылки для импорта
```bash
# Google Calendar
https://calendar.kuzyak.in/api/calendar/2024/ics
