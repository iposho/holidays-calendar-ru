...
1. Скачайте и разберите календарь:
   ...
   Если в `src/data/decrees.json` нет записи для года, скрипт создаст ее сам по странице consultant.ru:
   реквизиты постановления Правительства РФ «О переносе выходных дней», список переносов
   `{ "from": "...", "to": "..." }` и ссылку на текст с [publication.pravo.gov.ru](http://publication.pravo.gov.ru)
   (если документ не найден — на consultant.ru). Проект постановления не принимается.
   Если данные сайта не сходятся с переносами из постановления, скрипт завершится с ошибкой.
2. Запустите `npm test` — тесты сверяют каждый день года со снимком consultant.ru,
...
откроет PR в `develop`. Запись в `decrees.json` для нового года создается автоматически — проверьте ее в PR.
